...
vector<FLOW>flow;
vector<FLOW>flowEdge;
vector<FLOW>flowPod;
vector<FLOW>flowCore;
...
// Read input
void readInput(int& k, double& usec, double& scale, double& repRatio){
...
//	fprintf(stderr, "Input scale [1-100]: ");
	scanf("%lf", &scale);
//	fprintf(stderr, "Input ratio of repeated flows [0.0-1.0]: ");
	scanf("%lf", &repRatio);
//	fprintf(stderr, "Your input: (k = %d, t = %.3lf, sc = %.3lf, r = %.3lf)\n", k, usec, scale, repRatio);
}
...
// Generate flows
void gen(int k, double usec, double scale, double repRatio){

	// Variables
	int itmp, low, upp, sid;
	...
	// New flows
	curTime = 0.0;
	while(curTime < usec*(1.0-repRatio)){
		...
		// Classify
		if(typeOfFlow == 0) flowEdge.push_back(ftmp);
		else if(typeOfFlow == 1) flowPod.push_back(ftmp);
		else flowCore.push_back(ftmp);

		// Arrival time
		flowTime.push_back(curTime);
	...
	while(curTime < usec){
		...
		itmp = rand()%10;
		if(itmp < ratio[0]){
			if(!flowEdge.size()) continue;
			ftmp = flowEdge[ rand()%(flowEdge.size()) ];
			edgeCnt ++;
		}

		// Same Pod
		else if(itmp < ratio[0] + ratio[1]){
			if(!flowPod.size()) continue;
			ftmp = flowPod[ rand()%(flowPod.size()) ];
			aggrCnt ++;
		}

		// Out to core
		else{
			if(!flowCore.size()) continue;
			ftmp = flowCore[ rand()%(flowCore.size()) ];
			coreCnt ++;
		}
...
{
	int k;
	double usec, scale, repRatio;

	// Read input
	readInput(k, usec, scale, repRatio);

	// Initialize
	init();

	// Generate flow
	gen(k, usec, scale, repRatio);
